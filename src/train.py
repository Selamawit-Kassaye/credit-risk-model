...
# """
# train.py - Train a credit risk model and track with MLflow using real CSV data
# """
...
# def load_data():
#     """
#     Load the credit CSV data and prepare it for training.
...
#     # For simplicity, we'll use these as features for Logistic Regression
#     X = customer_agg.drop("CustomerId", axis=1)

#     # ============================================================
#     # CREATE TARGET VARIABLE
#     # ============================================================
#     # Placeholder: classify high-risk customers as those with negative total amount
    
#     y = (X["total_amount"] < 0).astype(int)

#     logger.info(f"Target distribution: 0 (low risk)={sum(y==0)}, 1 (high risk)={sum(y==1)}")
...
def load_data():
    """
    Load the real credit CSV data and prepare it for training.
    Returns:
        X: pd.DataFrame with features
        y: pd.Series with target
    """
    ...
    customer_agg = df.groupby("CustomerId").agg(
        ...
        currency_code=("CurrencyCode", "first"),
        country_code=("CountryCode", "first"),
        most_common_category=("ProductCategory", lambda x: x.mode()[0] if not x.mode().empty else "unknown"),
    ...
    customer_agg["recency_days"] = (snapshot_date - customer_agg["last_transaction"]).dt.days

    # ============================================================
    # Derived features
    # ============================================================
    customer_agg["amount_range"] = customer_agg["max_amount"] - customer_agg["min_amount"]
    customer_agg["amount_std_ratio"] = customer_agg["std_amount"] / (customer_agg["avg_amount"] + 1e-5)

    # Drop raw datetime columns
    customer_agg = customer_agg.drop(columns=["first_transaction", "last_transaction"])

    # ============================================================
    # Target variable (placeholder)
    # Customers with negative total_amount labeled as high risk
    # Later you can replace with RFM + KMeans clustering
    # ============================================================
    y = (customer_agg["total_amount"] < 0).astype(int)

    # Features DataFrame
    X = customer_agg.drop(columns=["CustomerId"])
